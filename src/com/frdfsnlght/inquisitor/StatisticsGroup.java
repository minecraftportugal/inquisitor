...
public final class StatisticsGroup {
    ...
    public void removeStatistic(Statistic statistic) {
        synchronized (statistics) {
            if (! statistics.containsKey(statistic.getName())) return;
            ...
            //synchronized (statistics) {
                ...
                for (String statName : statistics.keySet()) {
                    if (statistics.get(statName).isMapped()) {
                        if (! addedMapped) {
                            addedMapped = true;
                            sql.append('`').append(Statistic.MappedObjectsColumn).append("`,");
                        }
                    } else
            ...
            if (stat.isMapped()) {
                ...
                switch (stat.getType()) {
                    ...
                    case BOOLEAN:
                        ...
                        break;
                    case OBJECT:
                        values.set(statName, DB.decodeFromJSON(rs.getClob(statName)));
                        break;
                    case TIMESTAMP:
    ...
    public void validate() {
        ...
        try {
            ...
            if (! DB.tableExists(name)) {
                ...
                sql.append("UNIQUE KEY `").append(keyName).append("` (`").append(keyName).append("`)");
            ...
            if (! rs.next()) {
                ...
                if (oldNames != null)
                    for (String oldName : oldNames) {
                        ...
                        sql = new StringBuilder();
                        sql.append("ALTER TABLE ").append(DB.tableName(name));
                        sql.append(" CHANGE COLUMN `").append(oldName).append("` ");
                        sql.append(colDef.toString());
                        stmt = DB.prepare(sql.toString());
                        stmt.executeUpdate();
                        Utils.info("Renamed statistic column '%s' to '%s' for statistic group '%s'.", oldName, colName, name);
                        renamed = true;
                        break;
                    }
                ...
                if (! renamed) {
                    ...
                    stmt.executeUpdate();
                    stmt.close();
                    Utils.info("Added statistic column '%s' for statistic group '%s'.", colName, name);
                }
                ...
                int dbSize = -1;
                if (pos != -1) {
                    int pos2 = Math.max(dbType.indexOf(')', pos + 1), dbType.indexOf(',', pos + 1));
                    try {
                        dbSize = Integer.parseInt(dbType.substring(pos + 1, pos2));
                    } catch (NumberFormatException nfe) {}
                    dbType = dbType.substring(0, pos);
                }
                if ((! dbType.equals(type.getSQLType())) || ((size != -1) && (dbSize != size))) {
                    sql = new StringBuilder();
                    sql.append("ALTER TABLE ").append(DB.tableName(name));
                    ...
                    stmt = DB.prepare(sql.toString());
                    stmt.executeUpdate();
                    Utils.info("Altered statistic column '%s' for statistic group '%s'.", colName, name);
                }
            }
            return true;
